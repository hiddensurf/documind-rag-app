...
async def run_advanced_analysis(conv_id: str, request: ChatRequest):
    ...
    try:
        ...
        # Get query from request (support both 'query' and 'message' fields)
        ...
        if selected_model not in analyzer.MODELS:
            ...
            selected_model = 'nvidia/nemotron-nano-12b-v2-vl:free'
...
"""
        ...
        user_message = Message(
            role="user",
            content=f"✨ Advanced Vision Analysis with {model_name}: {query}",
            timestamp=datetime.now().isoformat()
        )
        conversation_service.add_message(conv_id, user_message)
        ...
        return {
            ...
            "response": combined_response,  # Backwards compatibility
            "sources": [],
            "timestamp": assistant_message.timestamp
        }
...
async def run_hybrid_analysis(conv_id: str, request: ChatRequest):
    ...
    try:
        ...
        # Extract model selection (default: Llama 3.3)
        selected_model = request.model or 'meta-llama/llama-3.3-70b-instruct:free'
        ...
        for doc in cad_docs:
            ...
            logger.info(f"Running hybrid analysis for: {doc_name} with {selected_model}")
            ...
            if not Path(png_path).exists():
                logger.warning(f"PNG not found for {doc_id}, skipping")
                all_analyses.append(f"⚠️ **{doc_name}**: Analysis PNG not available.")
                continue
...
## CV Detection
- Shapes: {cv_summary['total_shapes']}
- Lines: {cv_summary['total_lines']}
- Complexity: {cv_summary['complexity'].upper()}
...
"""
                ...
                logger.error(f"Hybrid analysis failed for {doc_id}: {e}")
                all_analyses.append(f"❌ **{doc_name}**: {str(e)}")
        ...
        user_message = Message(
            role="user",
            content=f"🤖 Hybrid AI Analysis: {query}",
            timestamp=datetime.now().isoformat()
        )
        conversation_service.add_message(conv_id, user_message)
...
async def run_hybrid_analysis(conv_id: str, request: ChatRequest):
    ...
    try:
        ...
        # Format response
        response_text = "🔬 HYBRID CAD ANALYSIS COMPLETE\n\n"
        for analysis in all_analyses:
            if 'error' in analysis:
                response_text += f"❌ {analysis['filename']}: {analysis['error']}\n\n"
            else:
                response_text += f"✅ {analysis['filename']}\n"
